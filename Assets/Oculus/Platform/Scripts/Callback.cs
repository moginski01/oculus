...
namespace Oculus.Platform
{
    public static class Callback
    {
        ...
        {
            if (callback == null) throw new Exception("Cannot provide a null notification callback.");
        ...
        private static readonly Dictionary<ulong, Request> requestIDsToRequests = new();
        private static readonly Dictionary<Message.MessageType, RequestCallback> notificationCallbacks = new();

        private static bool hasRegisteredRoomInviteNotificationHandler;
        private static readonly List<Message> pendingRoomInviteNotifications = new();

        private static void FlushRoomInviteNotificationQueue()
        {
            hasRegisteredRoomInviteNotificationHandler = true;
            foreach (var msg in pendingRoomInviteNotifications) HandleMessage(msg);
        ...
        private static void FlushJoinIntentNotificationQueue()
        {
            ...
            if (latestPendingJoinIntentNotifications != null) HandleMessage(latestPendingJoinIntentNotifications);
        ...
        private class RequestCallback
        {
            ...
            {
                if (messageCallback != null) messageCallback(msg);
            }
        }
        ...
        internal static void HandleMessage(Message msg)
        {
            ...
            else if (!hasRegisteredRoomInviteNotificationHandler &&
                     msg.Type == Message.MessageType.Notification_Room_InviteAccepted)

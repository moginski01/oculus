...
public class BufferedAudioStream
{
      ...
      Debug.Log(string.Format("current time: {0}, remainingBufferTime: {1}", Time.time, remainingBufferTime));
          ...
          Debug.Log("Starting playback");
            ...
            if (audio.isPlaying)
            {
                ...
                if (remainingBufferTime < 0) remainingBufferTime = 0;
            }
        }
        ...
        if (remainingBufferTime <= 0)
        {
            ...
            {
                if (writePos != 0) Debug.LogError("writePos non zero while not playing, how did this happen?");
            }
        }
    }
    ...
    {
        var remainingWriteLength = samples.Length;

        if (writePos > audioBuffer.Length) throw new Exception();

        do
        {
            var writeLength = remainingWriteLength;
            var remainingSpace = audioBuffer.Length - writePos;

            if (writeLength > remainingSpace) writeLength = remainingSpace;
            ...
            if (writePos > audioBuffer.Length) throw new Exception();
            if (writePos == audioBuffer.Length) writePos = 0;
